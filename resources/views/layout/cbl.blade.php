...
<div class="container-fluid mt-2">
    ...
    <h3 class="mb-3" style="font-family: serif; color: #312f2f; text-transform: uppercase">CITY BANK DATA</h3>
        ...
        <div class="card-body table-responsive">
            @if($data && $data->count() > 0)
                <div class="table-responsive">
                    <table id="cblTable" class="table table-striped table-hover">
                            ...
                            @foreach ($data as $key => $item)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        {{-- Delete --}}
                                        <form action="{{ route('cbl.destroy', $item->id) }}" method="POST"
                                            class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                ...
                                <div class="modal fade" id="editModal{{ $item->id }}" tabindex="-1" role="dialog"
                                    ...
                                    <div class="modal-dialog modal-lg" role="document">
                                        <div class="modal-content">
                                            <form method="POST" action="{{ route('cbl.update', $item->id) }}">
                                                @csrf
                                                @method('PUT')
                                ...
                                <div class="modal fade" id="assignModal{{ $item->id }}" tabindex="-1" role="dialog" aria-hidden="true">
                                    <div class="modal-dialog modal-lg" role="document">
                                        <div class="modal-content">
                                            <form action="{{ route('cbl.assign', $item->id) }}" method="POST">
                                                @csrf
...
<div class="modal fade" id="createServiceModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <form method="POST" action="{{ route('cbl.store') }}">
                @csrf
...
<div class="modal fade" id="importCSVModal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <form method="POST" action="{{ route('cbl.import') }}" enctype="multipart/form-data">
                @csrf
                <div class="modal-header bg-primary text-white">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const cblTable = document.getElementById('cblTable');

    if (searchInput && cblTable) {
        const rows = cblTable.querySelectorAll('tbody tr');
